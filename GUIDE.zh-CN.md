...
如果你只是想先走一遍不依赖真实模型的 smoke path，可以临时把
`[llm].provider = "mock"`，或者只把某个 agent 的 provider 改成 `mock`。
...
- `ollama` -> 默认 `base_url = "http://127.0.0.1:11434"`，不要求 API key

如果选了真实 provider，但缺少对应凭证，Close-Devs 会直接 fail fast，
不会再静默回退到 `mock`。
...
- 启动日志里会明确打印每个 agent 的 `provider=...` 和 `client=...`
- `summary.md` 或 `report.json` 里能看到 `actual_llm_provider` / `actual_llm_providers`
- summary 和 handoff 里有明显模型驱动的语义结论，而不只是 deterministic 工具输出

如果本次实际用的是 `mock`，运行时元数据会直接写出来。
...
- workflow 在启动前就失败
- 错误里直接写缺少某个 API key 环境变量
- 当前配置的 provider 是 `openai` 或 `openai_compatible`
